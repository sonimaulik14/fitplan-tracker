// "12-Week Muscle Builder" — the full program data, one const per week.
// Week arrays are the single source of truth for the plan's prescriptions;
// prisma/seed.ts syncs them into the database idempotently.
import {
  type Day,
  type ProgramDef,
  ex,
...
export const PLAN_NAME = "12-Week Muscle Builder";
...
const RAW_WEEKS: Record<number, Day[]> = {
  1: week1,
  2: week2,
  3: week3,
  4: week4,
...
// The retired add-morning-cardio.ts patch prepended Morning Cardio to EVERY
// day of this program — training days and rest days alike. Folded in here:
// Morning Cardio takes orderIndex 0 and every other exercise shifts +1.
export const WEEKS: Record<number, Day[]> = Object.fromEntries(
  Object.entries(RAW_WEEKS).map(([num, days]) => [
    num,
    days.map((day) => ({
      ...day,
      exercises: [morningCardio, ...day.exercises],
    })),
  ])
) as Record<number, Day[]>;

// Training style/protocol per week (shown as "Week 1 : YT3").
export const STYLES: Record<number, string> = {
  1: "YT3",
  2: "Y3T",
  3: "Y3T",
  4: "FST-7",
...
export const muscleBuilder: ProgramDef = {
  name: PLAN_NAME,
  description:
